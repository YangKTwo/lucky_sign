...
@Service
public class DrawMissSettler {
    private static final Logger log = LoggerFactory.getLogger(DrawMissSettler.class);
    ...
    public DrawMissSettler(DailyDrawRepository dailyDrawRepository,
    ...
    @Transactional
    public void settleDraw(Long drawId) {
        DailyDraw draw = dailyDrawRepository.findById(drawId).orElse(null);
        if (draw == null) {
            return;
        }
        ...
        draw.setStatus(DrawStatus.EXPIRED);
        dailyDrawRepository.save(draw);
        log.info("Settled miss userId={} date={} missStreak={}", user.getId(), draw.getDrawDate(), user.getMissStreakDays());
    }
}
